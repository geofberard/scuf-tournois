...
const loadTeams = (driveKey: string, setTeamsData: (data: Team[]) => void) => ({
  load: () => {
    // @ts-ignore
    const query = new google.visualization.Query(getUrl(driveKey, "Teams"));
    query.send(response => {
...
const loadGames = (driveKey: string, setGamesData: (data: Game[]) => void) => ({
  load: () => {
    ...
    // @ts-ignore
    const query = new google.visualization.Query(getUrl(driveKey, "Games"));
    query.send(response => {
      const driveData = response.getDataTable();
      const gamesData: Game[] = [];
      for (let i = 0; i < driveData.getNumberOfRows(); i++) {
        const dateArray = driveData.getValue(i, 0);
        const teamA = driveData.getValue(i, 2) as string;
        const teamB = driveData.getValue(i, 3) as string;
        const hours = dateArray.getHours();
        const minutes = dateArray.getMinutes();
        gamesData.push({
          id: `${dateArray[0]}${dateArray[1]}${teamA}${teamB}`,
          time: new Date(today.getFullYear(), today.getMonth(), today.getDate(), hours, minutes, 0, 0),
...
const loadRules = (driveKey: string, setGamesData: (data: Rule[]) => void) => ({
  load: () => {
    // @ts-ignore
    const query = new google.visualization.Query(getUrl(driveKey, "Rules"));
    query.send(response => {
      const driveData = response.getDataTable();
